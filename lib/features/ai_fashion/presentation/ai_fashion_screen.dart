...
class _AiFashionScreenState extends ConsumerState<AiFashionScreen> {
  ...
  Future<void> _handlePickImage() async {
    ...
    if (result != null) {
      ...
      try {
        final url = await ref.read(uploadRepositoryProvider).uploadFile(result.path);
  ...
  Future<void> _handleStartStyling() async {
    if (_isUploading) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please wait, photo is still uploading...')),
      );
      return;
    }
    ...
    if (_localPhotoUri != null && _uploadedPhoto == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Image upload failed or incomplete. Please try selecting the image again.')),
      );
      return;
    }
    ...
    final vibeSelection = _vibeSelections.isEmpty ? null : _vibeSelections.firstWhere((v) => v.key == 'styleTrendyElements', orElse: () => const widgets.FashionOptionItem(label: '', value: ''));
    ...
    await ref.read(aiFashionControllerProvider.notifier).generate(
          ...
          productImageUrl: _uploadedPhoto, // In RN it uses referenceImages: uploadedPhoto ? [uploadedPhoto] : undefined
  ...
  void _showOccasionSheet(List<AiFashionSelectionOption> options) {
    final group = options.firstWhere((o) => o.key.toLowerCase() == 'occasion', orElse: () => const AiFashionSelectionOption(key: 'occasion', options: []));
      ...
      builder: (context) => widgets.FashionOptionSheet(
        ...
        groups: [
          widgets.FashionOptionGroup(
            key: 'occasion',
            data: group.options.map((o) => widgets.FashionOptionItem(label: o.label, value: o.value, key: 'occasion')).toList(),
          ),
        ],
        initialSelected: _occasionText.isNotEmpty ? [widgets.FashionOptionItem(label: _occasionText, value: '', key: 'occasion')] : [],
        onConfirm: (selected) {
  ...
  void _showVibeSheet(List<AiFashionSelectionOption> options) {
    final styleGroup = options.firstWhere((o) => o.key == 'style', orElse: () => const AiFashionSelectionOption(key: 'style', options: []));
      ...
      builder: (context) => widgets.FashionOptionSheet(
        ...
        groups: [
          widgets.FashionOptionGroup(
            ...
            data: styleGroup.options.map((o) => widgets.FashionOptionItem(label: o.label, value: o.value, key: 'styleTrendyElements')).toList(),
          ),
        ],
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(aiOutfitPresetProvider((taskId: widget.taskId, templateId: widget.templateId)), (prev, next) {
      if (next.hasValue && next.value != null) {
        ...
        if (preset.styleTrendyElements != null && _vibeText.isEmpty) {
          setState(() {
            ...
            _vibeSelections = [widgets.FashionOptionItem(label: preset.styleTrendyElements!, value: preset.styleTrendyElements!, key: 'styleTrendyElements')];
          });
        }
      }
    });
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Style Me Today', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        backgroundColor: Colors.white,
        elevation: 0,
        actions: [
          IconButton(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              child: Row(
                ...
                children: [
                  ...
                  if (_selectedModel != null)
                    GestureDetector(
                      onTap: () => context.push(RoutePaths.fashionModelSettings),
                      child: Stack(
                        children: [
                          ClipRRect(
                            borderRadius: BorderRadius.circular(12),
                            child: CachedNetworkImage(
                              imageUrl: _selectedModel!.headerImageUrl ?? _selectedModel!.imageUrl,
                              width: 70,
                              height: 70,
                              fit: BoxFit.cover,
                            ),
                  ...
                  GestureDetector(
                    ...
                    child: _localPhotoUri != null
                        ...
                        : Container(
                            ...
                            child: const Icon(Icons.image_outlined, color: Color(0xFFFF6B6B), size: 24),
                          ),
                  ),
            ...
            // Occasion Section
            ThemedText('Where are you going?', type: ThemedTextType.defaultSemiBold),
            const SizedBox(height: 8),
            GestureDetector(
              onTap: () => optionsAsync.whenData((options) => _showOccasionSheet(options)),
              child: Container(
                  ...
                  children: [
                    Expanded(
                      child: Text(
                        ...
                        style: TextStyle(
                          color: _occasionText.isEmpty ? const Color(0xFF999999) : Colors.black,
                          fontSize: 15,
                        ),
                      ),
                    ),
            ...
            // Vibe Section
            ThemedText("What's the vibe?", type: ThemedTextType.defaultSemiBold),
            const SizedBox(height: 8),
            GestureDetector(
              onTap: () => optionsAsync.whenData((options) => _showVibeSheet(options)),
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                decoration: BoxDecoration(
                  color: Colors.white,
                  ...
                  children: [
                    Expanded(
                      child: Text(
                        ...
                        style: TextStyle(
                          color: _vibeText.isEmpty ? const Color(0xFF999999) : Colors.black,
                          fontSize: 15,
                        ),
                      ),
                    ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: controllerState.isLoading ? null : _handleStartStyling,
                ...
                child: controllerState.isLoading
                    ...
                    : const Text('Create My Look', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              ),
            ),
