...
abstract class SwaggerModelsGenerator extends SwaggerGeneratorBase {
  ...
  String generateFlexibleJsonKeyArgumentsForType(String typeName, {bool isList = false}) {
    if (typeName.isEmpty) {
      return '';
    }

    var normalizedType = typeName.replaceAll('?', '');
    if (normalizedType.startsWith('List<') && normalizedType.endsWith('>')) {
      normalizedType = normalizedType.substring(5, normalizedType.length - 1);
      isList = true;
    }

    if (normalizedType.startsWith('enums.')) {
      return '';
    }

    switch (normalizedType) {
      case 'int':
        return isList ? ', fromJson: JsonCoerce.asIntList' : ', fromJson: JsonCoerce.asInt';
      case 'double':
        return isList
            ? ', fromJson: JsonCoerce.asDoubleList'
            : ', fromJson: JsonCoerce.asDouble';
      case 'num':
        return isList ? ', fromJson: JsonCoerce.asNumList' : ', fromJson: JsonCoerce.asNum';
      case 'bool':
        return isList ? ', fromJson: JsonCoerce.asBoolList' : ', fromJson: JsonCoerce.asBool';
      case 'String':
        return isList
      ...
      case 'DateTime':
        return isList
            ? ', fromJson: JsonCoerce.asDateTimeList'
            : ', fromJson: JsonCoerce.asDateTime';
      default:
        return '';
    }
  }
...
''';
  ...
  String generatePropertyContentByDefault({
    ...
    final flexibleJsonKeyValue = generateFlexibleJsonKeyArgumentsForType(typeName);
    ...
    final jsonKeyContent =
        "@JsonKey(name: '$propertyKey'$includeIfNullString$dateToJsonValue${unknownEnumValue.jsonKey}$flexibleJsonKeyValue)\n";
...
static $returnType $fromJsonFunction($valueType? value) => $enumNameCamelCase$fromJsonSuffix(value, $defaultValueString);
  ...
  String generatePropertyContentBySchema(
    ...
    final jsonKeyContent =
        "@JsonKey(name: '${_validatePropertyKey(propertyKey)}'$includeIfNullString${unknownEnumValue.jsonKey}$dateToJsonValue$flexibleJsonKeyValue)\n";
  ...
  String generatePropertyContentByAnyOf({
    ...
    final includeIfNullString = generateIncludeIfNullString();
    final flexibleJsonKeyValue = generateFlexibleJsonKeyArgumentsForType(finalTypeName);
  ...
  String generatePropertyContentByAllOf({
    ...
    final jsonKeyContent =
        "@JsonKey(name: '${_validatePropertyKey(propertyKey)}'$includeIfNullString${unknownEnumValue.jsonKey}$flexibleJsonKeyValue)\n";
  ...
  String generateListPropertyContent({
    ...
    final flexibleJsonKeyValue =
        generateFlexibleJsonKeyArgumentsForType(typeName, isList: true);
    ...
    if (unknownEnumValue.jsonKey.isEmpty) {
      if (options.classesWithNullabeLists.any((element) => RegExp(element).hasMatch(className))) {
        jsonKeyContent =
            "@JsonKey(name: '$validatedPropertyKey'$includeIfNullString$flexibleJsonKeyValue)\n";
      } else {
        jsonKeyContent =
            "@JsonKey(name: '$validatedPropertyKey'$includeIfNullString$flexibleJsonKeyValue, defaultValue: <$typeName>[])\n";
      }
    } else {
      jsonKeyContent =
          "@JsonKey(name: '$validatedPropertyKey'$includeIfNullString${unknownEnumValue.jsonKey}$flexibleJsonKeyValue)\n";
    }
  ...
  String generateGeneralPropertyContent({
    ...
    jsonKeyContent += flexibleJsonKeyValue;
